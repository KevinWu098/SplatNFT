log_time() {
  ...
  if ! "$@"; then
    echo "Error during: $description" >&2
    exit 1
  fi
...
log_time "Cleanup timing file" rm -f timing.txt
...
log_time "Extract frames from video" ffmpeg -i lia_format.MOV -vf fps=2 data/input/frame_%04d.png

log_time "Convert frames" xvfb-run python gaussian-splatting/convert.py -s data

log_time "Train model" xvfb-run python gaussian-splatting/train.py -s data -m trained_model --iterations 7000
